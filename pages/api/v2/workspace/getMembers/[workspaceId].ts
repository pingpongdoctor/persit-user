...
import { WorkspaceModel } from 'mongo/schema';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      const workspaceResult = await WorkspaceModel.findOne({ id: workspaceId?.toString() });
      if (!workspaceResult) {
        res.status(404).json({ message: 'Workspace not found' });
      } else {
        res.status(200).json({ message: 'Fetched workspace members', data: workspaceResult?.members });
      }
    } catch (error) {
      console.error(error);
